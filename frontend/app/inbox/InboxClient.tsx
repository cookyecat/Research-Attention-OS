...
export default function InboxPage() {
  ...
  useEffect(() => {
    ...
    if (!query) { setLibraryMatchIds(null); return; }
    let cancelled = false;
    const timer = window.setTimeout(async () => {
      try {
        const matches = await api<Source[]>(`/sources/search?q=${encodeURIComponent(query)}&limit=200`);
        if (!cancelled) setLibraryMatchIds(new Set(matches.map((item) => item.id)));
      } catch {
        if (!cancelled) setLibraryMatchIds(null);
      }
    }, 140);
  ...
  const filteredRecent = useMemo(() => recent.filter((source) => {
    ...
    if (libraryMatchIds) return libraryMatchIds.has(source.id);
  ...
  }), [recent, originFilter, stateFilter, libraryQuery, libraryMatchIds, attentionBySource]);
