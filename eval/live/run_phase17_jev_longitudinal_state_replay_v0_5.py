...
from app.services.event_state_fact_delta import (
    FACT_APPLIER_CONTRACT,
    FACT_DECIDER_CONTRACT,
    FACT_DELTA_CONTRACT,
    FACT_STATE_CONTRACT,
    FactDeltaV01,
...
RUN_VERSION = "phase17-jev-longitudinal-state-replay-v0.5"
...
OUT_DIR = ROOT / "eval/live/results/phase17_jev_longitudinal_state_replay_v0_5"
DEFAULT_CHECKPOINT = OUT_DIR / "phase17_jev_longitudinal_state_replay_v0.5_checkpoint.json"
...
def _checkpoint_identity(
    *,
    ...
    return {
        ...
        "decider_contract": FACT_DECIDER_CONTRACT,
        "fact_delta_contract": FACT_DELTA_CONTRACT,
        "fact_state_contract": FACT_STATE_CONTRACT,
        "applier_contract": FACT_APPLIER_CONTRACT,
        "execution_contract": "persisted-fact-delta-apply-v0.1",
...
def run(
    *,
    ...
    try:
        ...
        previous_fact_state: SemanticFactStateV01 | None = None
        ...
        # Deterministically reconstruct the prefix from persisted admitted FactDelta records.
        ...
        for ordinal, persisted in enumerate(persisted_trajectory, start=1):
            ...
            persisted_delta = persisted.get("fact_delta")
            delta = (
                FactDeltaV01.model_validate(persisted_delta)
            ...
            applied = apply_fact_delta(
                db,
                event_id=event.id,
                previous=previous_fact_state,
                observation=observation,
                delta=delta,
                supporting_source_ids=supporting_source_ids,
            )
            next_fact_state = applied.fact_state
            next_state = applied.event_state
            expected_fact_digest = persisted["fact_state_digest"]
            if next_fact_state.state_digest != expected_fact_digest:
                raise RuntimeError(
                    "Deterministic Fact Apply replay digest mismatch at ordinal "
                    f"{ordinal}: {next_fact_state.state_digest} != {expected_fact_digest}"
                )
            if next_fact_state.model_dump(mode="json") != persisted["fact_state"]:
                raise RuntimeError(
                    f"Deterministic Fact Apply replay state mismatch at ordinal {ordinal}"
                )
            ...
            previous_fact_state = next_fact_state
        ...
        for ordinal, row in enumerate(
            ...
            if not new_units:
                if previous_fact_state is None:
                    raise RuntimeError(
                ...
                if previous_fact_state is not None:
                    previous_support_refs = sorted(
                        {
                            ref
                            for fact in previous_fact_state.facts
                    ...
                    if missing_previous_support:
                        raise RuntimeError(
                            "Missing dereferenced semantic units for previous current-fact support: "
            ...
            trajectory.append(
                {
                    ...
                    "fact_state": next_fact_state.model_dump(mode="json"),
                    "fact_state_digest": next_fact_state.state_digest,
                    "current_fact_count": len(next_fact_state.facts),
            ...
            print(
                ...
                "/",
                len(hydrated_rows),
                (delta.kind if delta is not None else "NONE"),
                "new_units",
                len(new_units),
                "facts",
                len(next_fact_state.facts),
                "active_refs",
        ...
        for row in trajectory:
            delta_counts[row["delta_kind"]] = (
                delta_counts.get(row["delta_kind"], 0) + 1
            )
        ...
        return {
            ...
            "summary": {
                ...
                "final_fact_state": (
                    previous_fact_state.model_dump(mode="json")
                    if previous_fact_state is not None
                    else None
                ),
                "final_state": (
                ...
                "final_current_fact_count": (
                    len(previous_fact_state.facts)
                    if previous_fact_state is not None
                    else 0
                ),
                ...
                "max_current_fact_count": max(
                    (row["current_fact_count"] for row in trajectory),
                    default=0,
                ),
            ...
            "guardrails": [
                ...
                "Configured real LLM is used only for new CurrentFact Decide calls; Fact Apply is deterministic.",
                "Each successful observation is durably checkpointed with the admitted FactDelta or explicit NONE plus materialized fact/event states.",
                "Resume replays persisted FactDelta records through deterministic Fact Apply and verifies both fact-state and EventState digests before any new LLM call.",
                "Checkpoint resume is rejected if projection digest or Decide/FactDelta/FactState/Apply contract identity changes.",
