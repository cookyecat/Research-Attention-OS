...
from app.services.event_state_slot_projection_v13 import (
    decide_slot_delta_two_stage_with_audit,
)

RUN_VERSION = "phase17-state-slot-projection-v13-real-model-v0.1"
OUT_DIR = ROOT / "eval/live/results/phase17_state_slot_projection_v13_real_model_v0_1"
def _db():
...
def _run_case(
    *,
    ...
    try:
        ...
        decision=decide_slot_delta_two_stage_with_audit(
        ...
        created_families=sorted(
            m.slot.primitive_family for m in mutations if m.mode=="CREATE"
        )
        expected_families=(
            sorted(expect_created_families)
            if expect_created_families is not None
            else None
        )
        created_family_match=(
            True
            if expected_families is None
            else created_families == expected_families
        )
        ...
        return {
            ...
            "question_constraint_match": not forbidden_hits,
            "created_families": created_families,
            "expect_created_families": expected_families,
            "created_family_match": created_family_match,
            "technical_error": None,
            "proposition_basis": [
                row.model_dump(mode="json")
                for row in decision.proposition_basis
            ],
...
def run():
    ...
    cases.append(_run_case(
        name="APPLICATION_WORKFLOW_RESTATEMENT_NONE",
        previous_slots=[
            _slot(
                "slot-capability",
                "capability",
                "What capabilities or behaviors has the system demonstrated?",
                "The system has demonstrated reading market data, deciding Buy or Sell, submitting trades, and repeating this decision loop in an automated trading bot.",
                ("uTradeCap",),
        ...
        previous_units=[{
            "unit_id":"uTradeCap",
            "statement":"The system has demonstrated reading market data, deciding Buy or Sell, submitting trades, and repeating this decision loop in an automated trading bot."
        }],
        new_units=[{
            "unit_id":"uTradeDetail",
            "statement":"In the same demonstrated bot, MON/USDC market data arrives, the system judges Buy or Sell, sends the order to Kuru, then repeats when new market data arrives."
        }],
        expect_mutations=1, expect_create=0, expect_existing=1, expect_slot_count=1,
        expect_created_families=(),
    ))
    ...
    cases.append(_run_case(
        name="APPLICATION_WORKFLOW_FIRST_SEEN_AS_CAPABILITY",
        previous_slots=[],
        previous_units=[],
        new_units=[{
            "unit_id":"uWorkflow",
            "statement":"In a live automated bot, the system repeatedly reads market data, decides Buy or Sell, submits the order, and repeats the behavior when new data arrives."
        }],
        expect_mutations=2, expect_create=2, expect_existing=0, expect_slot_count=2,
        expect_status="EMERGING",
        forbid_created_question_terms=("trading","market","order","bot","workflow","kuru","mon/usdc"),
        expect_created_families=("DISPOSITION","RELATION"),
    ))
    ...
    diagnostics["all_pass"]=all(
        ...
        and r["question_constraint_match"]
        and r["created_family_match"]
        for r in cases
    )
