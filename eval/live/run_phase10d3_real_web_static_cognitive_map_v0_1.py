...
RUN_VERSION = "phase10d3-real-web-static-cognitive-map-v0.1"
OUT_DIR = ROOT / "eval/live/results/phase10d3_real_web_static_cognitive_map_v0_1"
MANIFEST = ROOT / "eval/live/manifest.phase10d3_real_web_broadening.v0.1.yaml"
LABELS = ("N3", "N4", "B3", "B4", "F3", "F4")
...
def prepare_batch3_world():
    manifest = yaml.safe_load(MANIFEST.read_text(encoding="utf-8")) or {}
    if manifest.get("selection_status") != "selected_by_acquisition_only_preflight":
        raise RuntimeError("batch3 selection status mismatch")
    specs = {str(row["label"]): row for row in manifest.get("sources") or []}
    if tuple(specs) != LABELS:
        raise RuntimeError(f"batch3 labels/order mismatch: {tuple(specs)}")
    ...
    try:
        for label in LABELS:
            spec = specs[label]
            base = {
                "label": label, "stratum": str(spec["stratum"]),
                "publisher_preregistered": str(spec["publisher"]),
                "title_preregistered": str(spec["title"]),
                "requested_url": str(spec["url"]),
                "expected_content_hash": str(spec["content_hash"]),
                "expected_content_chars": int(spec["content_chars"]),
            }
            try:
                source = ingest_url(db, str(spec["url"]))
            except Exception as exc:
                acquisition[label] = {**base, "status": "ACQUISITION_ERROR", "error_type": type(exc).__name__, "error": str(exc)[:2000]}
                ...
                continue
            chars = len(source.content_text or "")
            hash_match = source.content_hash == str(spec["content_hash"])
            char_match = chars == int(spec["content_chars"])
            acquisition[label] = {
                **base, "status": "OK" if hash_match and char_match else "SNAPSHOT_MISMATCH",
                "source_id": str(source.id), "title_fetched": source.title,
                "canonical_url": source.canonical_url, "actual_content_hash": source.content_hash,
                "actual_content_chars": chars, "hash_match": hash_match, "char_count_match": char_match,
                "fingerprint": source.fingerprint, "ingestion_method": source.ingestion_method,
                "raw_metadata": source.raw_metadata,
            }
            if hash_match and char_match:
                sources[label] = source
            else:
                print(json.dumps({"label": label, "stage": "SNAPSHOT_MISMATCH", "hash_match": hash_match, "char_count_match": char_match}, ensure_ascii=False), flush=True)
...
def main() -> int:
    ...
    engine, db, sources, acquisition, manifest = prepare_batch3_world()
    try:
        acquisition_class = {label: ("PREFLIGHT_SNAPSHOT_CONFIRMED" if label in sources else "ACQUISITION_ERROR") for label in LABELS}
        ...
        out = {
            ...
            "cases": cases,
            "guardrails": [
                "Batch-3 sources were selected by acquisition-only preflight before any RAOS cognitive outcome was observed.",
                "Every Batch-3 source must match the exact acquisition-preflight hash and character count before model calls.",
                "An acquisition or preflight-snapshot mismatch is retained as a source failure; remaining selected sources continue.",
