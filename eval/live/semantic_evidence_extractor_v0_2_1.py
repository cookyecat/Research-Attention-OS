"""Semantic Evidence Extractor v0.2.1 — temporal-anchor development revision.

Controlled revision over v0.2:
- keeps SemanticExtractionBatchV0_2 and all semantic/provenance structure unchanged;
- supplies pinned source publication/update/capture metadata;
- forbids measurement time from silently anchoring source-relative expressions;
- preserves unresolved source-relative time when no source-time anchor exists.
...
EXTRACTOR_VERSION = "semantic-evidence-extractor-v0.2.1"
PROMPT_VERSION = "semantic-evidence-extraction-v0.2.1"
...
Remember: measurement_as_of is NOT a source-time anchor. If source_published_at is unknown and the raw source supplies no valid date anchor, keep relative time relative to the source and leave absolute calendar time unresolved.
...
def estimate_semantic_evidence_v0_2_1(source: LoadedSemanticSource, *, as_of: str, chat_fn=None) -> dict[str, Any]:
    """Run one v0.2.1 development extraction with one structural repair and diagnostics."""
    ...
    repair_messages = list(messages) + [
        ...
        {"role": "user", "content": "Your JSON failed the required contract. Correct structural/provenance/temporal-binding errors only; do not invent unsupported evidence.\nErrors:\n" + json.dumps(schema_events[-1]["errors"], ensure_ascii=False) + "\nReturn corrected JSON only.\n\n" + COMPACT_OUTPUT_CONTRACT},
    ]
    try:
