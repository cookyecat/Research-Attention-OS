...
from app.services.event_state_delta import (
    STATE_APPLIER_CONTRACT,
    STATE_DECIDER_CONTRACT,
...
RUN_VERSION = "phase17-jev-longitudinal-state-replay-v0.4"
...
OUT_DIR = ROOT / "eval/live/results/phase17_jev_longitudinal_state_replay_v0_4"
DEFAULT_CHECKPOINT = OUT_DIR / "phase17_jev_longitudinal_state_replay_v0.4_checkpoint.json"
...
def _checkpoint_identity(
    *,
    ...
    return {
        ...
        "decider_contract": STATE_DECIDER_CONTRACT,
        "state_delta_contract": STATE_DELTA_CONTRACT,
        "applier_contract": STATE_APPLIER_CONTRACT,
        "execution_contract": "persisted-state-delta-apply-v0.1",
...
def run(
    *,
    ...
    try:
        ...
        # Deterministically reconstruct the prefix from persisted admitted StateDelta records.
        ...
        for ordinal, persisted in enumerate(persisted_trajectory, start=1):
            ...
            persisted_delta = persisted.get("state_delta")
            delta = (
                StateDeltaV01.model_validate(persisted_delta)
            ...
            applied = apply_state_delta(
                ...
                delta=delta,
                supporting_source_ids=supporting_source_ids,
                semantic_units_by_ref=semantic_unit_by_ref,
            )
        ...
        return {
            ...
            "guardrails": [
                ...
                "Configured real LLM is used only for new Decide calls; Apply is deterministic.",
                "Each successful observation is durably checkpointed with the admitted StateDelta or explicit NONE.",
                "Resume replays persisted StateDelta records through Apply and verifies state digests before any new LLM call.",
                "Checkpoint resume is rejected if projection digest or Decide/Delta/Apply contract identity changes.",
