"""Run Semantic Evidence Extractor v0.2 on selected development sources.

Development-only. Does not score D/S/P/A and does not create fresh validation evidence.
Preserves structured-output diagnostics on failure.
"""
...
DEFAULT_OUT_DIR = ROOT / "eval" / "live" / "results" / "semantic_evidence_dev_v0_2"
...
def main() -> None:
    ...
    rows = []
    for source in sources:
        result = estimate_semantic_evidence_v0_2(source, as_of=args.as_of)
    ...
    artifact = {
        "name": "raos-semantic-evidence-development-run-v0.2",
    ...
    out_path = args.out_dir / f"semantic_evidence_dev_v0_2_{timestamp}.json"
    if out_path.exists():
