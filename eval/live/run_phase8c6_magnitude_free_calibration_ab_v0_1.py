...
RUN_VERSION = "phase8c6-magnitude-free-calibration-ab-v0.1"
STEP2 = ROOT / (
...
OUT_DIR = ROOT / "eval/live/results/phase8c6_magnitude_free_calibration_ab_v0_1"
...
def _run_one(case_id: str, run: dict, manifest: dict) -> dict:
    ...
    try:
        ...
        magnitude_free_frontier = pareto_frontier(legal, matches, calibration_strategy=MAGNITUDE_FREE_CALIBRATION)
        return {
            "case": case_id,
            ...
            "raw_frontier": [
                {**_effect_row(effect), "decision_vector": list(decision_vector(effect, matches, calibration_strategy=RAW_CARDINAL_CALIBRATION))}
                for effect in raw_frontier
            ],
            "magnitude_free_frontier": [
                {**_effect_row(effect), "decision_vector": list(decision_vector(effect, matches, calibration_strategy=MAGNITUDE_FREE_CALIBRATION))}
                for effect in magnitude_free_frontier
            ],
            "raw_pareto": _strategy_result(get_decision_strategy("pareto-multidelta"), normalized, matches),
            "magnitude_free_pareto": _strategy_result(get_decision_strategy("pareto-multidelta-magnitude-free"), normalized, matches),
        }
    finally:
        db.close()
        engine.dispose()
...
def _summary(rows: list[dict]) -> dict:
    ...
    for case_id in CASES:
        ...
        raw = Counter(row["raw_pareto"]["disposition"] for row in case_rows)
        mf = Counter(row["magnitude_free_pareto"]["disposition"] for row in case_rows)
        changed = [row["repeat"] for row in case_rows if row["raw_pareto"]["disposition"] != row["magnitude_free_pareto"]["disposition"]]
        out[case_id] = {
            "raw_cardinal_attention": dict(raw),
            "magnitude_free_attention": dict(mf),
            "attention_changed_repeats": changed,
            "raw_frontier_sizes": [len(row["raw_frontier"]) for row in case_rows],
            "magnitude_free_frontier_sizes": [len(row["magnitude_free_frontier"]) for row in case_rows],
        }
    return out
...
def main() -> int:
                ...
                json.dumps(
                    {
                        "case": case_id,
                        "repeat": row["repeat"],
                        "raw_cardinal": row["raw_pareto"]["disposition"],
                        "magnitude_free": row["magnitude_free_pareto"]["disposition"],
                        "raw_frontier": row["raw_frontier"],
                        "magnitude_free_frontier": row["magnitude_free_frontier"],
                    },
                    ensure_ascii=False,
                ),
                flush=True,
    ...
    output = {
        ...
        "status": "CONTROLLED_FROZEN_CALIBRATION_AB",
        "measurement_sha": git_head(),
        "source_artifact": str(STEP2.relative_to(ROOT)),
        "source_artifact_sha256": STEP2_SHA256,
        "frozen_layers": ["native canonical CognitiveEffect topology", "raw epistemic/importance", "Kernel fixture", "runtime", "Pareto aggregation"],
        "changed_variable": "effect_calibration",
        ...
        "limitations": [
            ...
            "Arm A uses frozen raw LLM change_magnitude; Arm B stores it but grants it zero decision authority.",
            "Epistemic strength and target importance are held fixed; this experiment isolates magnitude authority only.",
        ],
        "summary": summary,
        "rows": rows,
    }
