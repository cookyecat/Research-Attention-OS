...
from eval.live.phase10d6h_cardinal_free_relation_v0_1 import (
    CONTRACT_VERSION,
    CardinalFreeResponse,
    SYSTEM_PROMPT,
    user_prompt,
)
...
RUN_VERSION = "phase10d6h-cardinal-field-removal-parity-v0.1"
OUT_DIR = ROOT / "eval/live/results/phase10d6h_cardinal_field_removal_parity_v0_1"
CASES = ("A", "D", "X", "N4")
REPEATS = 6
SOURCE_G = ROOT / "eval/live/results/phase10d6g_native_support_binding_parity_v0_1/phase10d6g_native_support_binding_parity_v0.1_20260911T070844Z.json"
SOURCE_G_SHA = "e4b9409414fc4c6f08cb5ba651eb1ad58dece7ff7636e0dd1d6582c2bfdaf63b"
G_STABLE_SENTINELS = {
    "A": [("CHALLENGE", "B1"), ("OPEN_NEW", "OPEN_NEW"), ("REINFORCE", "M1"), ("REINFORCE", "Q1")],
    "D": [("CHALLENGE", "B2"), ("OPEN_NEW", "OPEN_NEW"), ("REINFORCE", "B1"), ("REINFORCE", "BT1"), ("REINFORCE", "Q1"), ("REINFORCE", "Q2"), ("REINFORCE", "M1")],
    "X": [("CHALLENGE", "B1"), ("CHALLENGE", "BT1"), ("CHALLENGE", "Q1"), ("OPEN_NEW", "OPEN_NEW"), ("REINFORCE", "M1")],
    "N4": [("OPEN_NEW", "OPEN_NEW"), ("REINFORCE", "B1"), ("REINFORCE", "BT1"), ("REINFORCE", "M1"), ("REINFORCE", "Q1")],
}
...
def summarize(label, rows):
    ...
    sentinels = {}
    for family in G_STABLE_SENTINELS[label]:
        count = family_counts[family]
        sentinels[repr(family)] = {
            "sample_count": count,
            "sample_rate": count / len(ok) if ok else 0.0,
            "recovered_at_least_once": bool(count),
            "recovered_majority": count >= 4,
        }
    support = {}
    ...
    return {
        ...
        "relation_family_sample_counts": {repr(k): v for k, v in sorted(family_counts.items(), key=lambda x: repr(x[0]))},
        "g_stable_sentinel_recovery": sentinels,
        "g_stable_fraction_recovered_once": sum(int(v["recovered_at_least_once"]) for v in sentinels.values()) / len(sentinels),
        "g_stable_fraction_recovered_majority": sum(int(v["recovered_majority"]) for v in sentinels.values()) / len(sentinels),
        "support_binding_by_family": support,
    }


def main():
    if sha256(SOURCE_G) != SOURCE_G_SHA:
        raise RuntimeError("10D.6G canonical artifact SHA mismatch")
    ...
    out = {
        ...
        "status": "CARDINAL_FIELD_REMOVAL_PARITY_SHADOW",
        "measurement_sha": git_head(),
        "relation_contract_version": CONTRACT_VERSION,
        "source_phase10d6g": str(SOURCE_G.relative_to(ROOT)),
        "source_phase10d6g_sha256": SOURCE_G_SHA,
        "system_prompt_sha256": hashlib.sha256(SYSTEM_PROMPT.encode()).hexdigest(),
        "cases": list(CASES),
        "repeats_per_case": REPEATS,
        "g_stable_sentinels": {k: [list(x) for x in v] for k, v in G_STABLE_SENTINELS.items()},
        "results": results,
        "guardrails": [
            "Exact frozen Auditor worlds, Kernel fixtures and modal Locate are reused; no acquisition/Sensor/Auditor/Locate call occurs.",
            "Historical native relation semantics and 10D.6G provenance obligations are preserved.",
            "Only the three effect-level cardinal compatibility fields are removed from Relation-Mapping output.",
            ...
            "No Attention or authority policy is evaluated.",
