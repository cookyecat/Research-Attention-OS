"""Phase17.3-KS-E FULL vs pairwise address-locked KeyBy on frozen Jev v0.7."""
...
from app.services.event_state_address_locked_keyby import (
    semantic_keyby_pairwise_address_locked,
)
...
def run_locked(previous, observation, flatmap):
    return with_retries(lambda: semantic_keyby_pairwise_address_locked(
        event_identity=EVENT_IDENTITY,
        previous=previous,
        observation=observation,
        flatmap=flatmap,
...
def main():
    ...
    for index, step in enumerate(trajectory):
        ...
        locked = run_locked(previous, observation, flatmap)
        ...
        base = {
            ...
            "locked_attempts": locked["attempts"],
            "locked_errors": locked["errors"],
            "locked_latency_s": locked["latency_s"],
        }
        if full["value"] is None or locked["value"] is None:
            row = {
                **base,
                "status": "ERROR",
                "full_success": full["value"] is not None,
                "locked_success": locked["value"] is not None,
            }
            rows.append(row)
            print(json.dumps(row, ensure_ascii=False), flush=True)
            continue
        _full_delta, full_draft = full["value"]
        locked_result = locked["value"]

        frozen_sig = action_signature(frozen)
        full_sig = action_signature(full_draft)
        locked_sig = action_signature(locked_result.draft)

        pairwise_rows = locked_result.pairwise_plan.propositions
        row = {
            **base,
            "status": "PASS",
            "candidate_visible_slot_count": (
                locked_result.candidate_plan.visible_slot_count
            ),
            "candidate_compression_ratio": (
                locked_result.candidate_plan.compression_ratio
            ),
            ...
            "create_group_validation_valid": (
                locked_result.create_group_validation.valid
                if locked_result.create_group_validation is not None
                else True
            ),
            "create_group_check_count": (
                len(locked_result.create_group_validation.checks)
                if locked_result.create_group_validation is not None
                else 0
            ),
            "frozen_signature": frozen_sig,
            "full_signature": full_sig,
            "locked_signature": locked_sig,
            "semantic_gold_signature": SEMANTIC_GOLD[step["ordinal"]],
            "full_vs_frozen": full_sig == frozen_sig,
            "locked_vs_full": locked_sig == full_sig,
            "locked_vs_frozen": locked_sig == frozen_sig,
            ...
            "locked_vs_semantic_gold": (
                locked_sig == SEMANTIC_GOLD[step["ordinal"]]
            ),
            "address_policy": locked_result.address_policy.model_dump(
                mode="json"
            ),
            "pairwise_plan": locked_result.pairwise_plan.model_dump(
                mode="json"
            ),
            "full_draft": full_draft.model_dump(mode="json"),
            "locked_draft": locked_result.draft.model_dump(mode="json"),
        }
        rows.append(row)
        print(json.dumps({
            key: row[key]
    ...
    result = {
        "benchmark": "phase17.3-ks-e-address-locked-comparison-v0.6",
        ...
        "locked_vs_full_agreement": (
            sum(row["locked_vs_full"] for row in successful)
            / len(successful) if successful else None
        ),
        "locked_vs_frozen_agreement": (
            sum(row["locked_vs_frozen"] for row in successful)
            / len(successful) if successful else None
        ),
        ...
        "locked_vs_semantic_gold_accuracy": (
            sum(row["locked_vs_semantic_gold"] for row in successful)
            / len(successful) if successful else None
        ),
        ...
        "create_group_check_count": sum(
            row["create_group_check_count"] for row in successful
        ),
        "create_group_validation_pass_count": sum(
            row["create_group_validation_valid"] for row in successful
        ),
        "mean_full_latency_s": (
            sum(row["full_latency_s"] for row in successful)
            / len(successful) if successful else None
        ),
        "mean_locked_latency_s": (
            sum(row["locked_latency_s"] for row in successful)
            / len(successful) if successful else None
        ),
        "rows": rows,
    }
    ...
    print(json.dumps({
        ...
        "locked_vs_full_agreement": result["locked_vs_full_agreement"],
        "locked_vs_frozen_agreement": result["locked_vs_frozen_agreement"],
        ...
        "locked_vs_semantic_gold_accuracy": result["locked_vs_semantic_gold_accuracy"],
        ...
        "create_group_check_count": result["create_group_check_count"],
        "create_group_validation_pass_count": result["create_group_validation_pass_count"],
        "mean_full_latency_s": round(
            result["mean_full_latency_s"], 3
        ) if result["mean_full_latency_s"] is not None else None,
        "mean_locked_latency_s": round(
            result["mean_locked_latency_s"], 3
        ) if result["mean_locked_latency_s"] is not None else None,
