...
RUN_VERSION = "phase10d6l1-support-binding-diagnostic-v0.1"
OUT_DIR = ROOT / "eval/live/results/phase10d6l1_support_binding_diagnostic_v0_1"
CASES = ("A", "D", "X", "N4")
REPEATS = 6

PROVENANCE_ADDENDUM = """
For each effect that you would emit under the semantic instructions above, also bind provenance:
...
The compatibility numeric fields remain required by the response schema but are diagnostic only in this provenance diagnostic.
"""


def assert_prompt_delta():
    ...
    if expected != IMPACT_SYSTEM_PARETO_COMPAT:
        raise RuntimeError("P1 base prompt differs from P0 beyond preregistered deletions")
...
def run_one(label, arm, ordinal, case, nodes, matches):
    base = IMPACT_SYSTEM if arm == "P0" else IMPACT_SYSTEM_PARETO_COMPAT
    system = base + PROVENANCE_ADDENDUM
    parsed, meta, events = chat_json_schema(
        [{"role": "system", "content": system}, {"role": "user", "content": user_prompt(case["frozen_units"], matches, nodes)}],
        NativeSupportBoundResponse,
        chat_fn=forced_chat,
        thinking="disabled",
        timeout=60.0,
    )
    raw, valid_effects, invalid = [], [], []
    ...
    return {
        ...
        "raw_effects": raw, "valid_effects": valid_rows, "normalized_effects": norm_rows,
        "invalid_effects": invalid, "raw_effect_count": len(raw),
        "valid_effect_count": len(valid_effects), "normalized_effect_count": len(normalized),
...
def summarize(label, rows):
    ...
    anchors_by_family = defaultdict(Counter)
    for r in ok:
        families = set()
        for e in r["normalized_effects"]:
            ...
            anchors_by_family[family][tuple(sorted(e["jurisdiction_anchors"]))] += 1
    ...
    for family, counter in support_by_family.items():
        ...
        ac = anchors_by_family[family]
        anchor_sig, anchor_count = ac.most_common(1)[0]
        stable_support[repr(family)] = {
            "n_effects": total,
            "n_support_signatures": len(counter),
            "mode_signature": list(top_sig),
            "mode_rate": top_count / total,
            "n_anchor_signatures": len(ac),
            "anchor_mode_signature": list(anchor_sig),
            "anchor_mode_rate": anchor_count / total,
        }
    return {
        "n_ok": len(ok),
...
def main():
    ...
    for label in CASES:
        ...
        for ordinal in range(1, REPEATS + 1):
            for arm in ("P0", "P1"):
                try:
                    row = run_one(label, arm, ordinal, case, nodes, matches)
        ...
        results[label] = {
            ...
            "arms": arms,
            "summary": {arm: summarize(label, rows) for arm, rows in arms.items()},
        }
        print(json.dumps({"label": label, "summary": {
            arm: {k: v for k, v in results[label]["summary"][arm].items() if k != "support_binding_by_family"}
            for arm in ("P0", "P1")
        }}, ensure_ascii=False), flush=True)
    p0 = IMPACT_SYSTEM + PROVENANCE_ADDENDUM
    p1 = IMPACT_SYSTEM_PARETO_COMPAT + PROVENANCE_ADDENDUM
    out = {
        "run_version": RUN_VERSION,
        "status": "PREREGISTERED_SUPPORT_BINDING_DIAGNOSTIC",
        "measurement_sha": git_head(),
        "cases": list(CASES),
        "repeats_per_arm": REPEATS,
        "prompt_sha256": {"P0": hashlib.sha256(p0.encode()).hexdigest(), "P1": hashlib.sha256(p1.encode()).hexdigest()},
        "controlled_variable": "same_provenance_obligation_plus_delete_two_downstream_policy_instructions_only",
        "results": results,
        "guardrails": [
            "Follow-up diagnostic explicitly authorized by the 10D.6L.1 preregistration.",
            "Both arms receive the identical support/jurisdiction provenance addendum and identical canonical user payload/schema.",
            "The only P0/P1 base-prompt difference is the two preregistered downstream-policy deletions.",
            "Unknown support/anchor/target identifiers fail closed at effect level and are never rewritten.",
            "No Attention policy or production switch is evaluated in this diagnostic.",
