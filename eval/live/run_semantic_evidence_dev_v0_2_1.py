"""Run Semantic Evidence Extractor v0.2.1 on selected development sources.

Development-only. Controlled temporal-anchor revision over v0.2.
Does not score D/S/P/A and does not create fresh validation evidence.
"""
...
DEFAULT_OUT_DIR = ROOT / "eval" / "live" / "results" / "semantic_evidence_dev_v0_2_1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source", action="append", default=[], help="Development source id such as RS02")
...
def select_entries(args: argparse.Namespace) -> list[dict]:
    ...
    unknown = [sid for sid in args.source if sid not in by_id]
    if unknown:
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        for source in sources:
            ...
            rows.append({**source_metadata(source), "message_chars": message_chars, "rough_token_estimate_chars_div_4": round(message_chars / 4)})
        print(json.dumps({"dry_run": True, "n_sources": len(rows), "as_of": args.as_of, "prompt_sha256": prompt_sha256(), "sources": rows, "note": "No source truncated; no model called."}, ensure_ascii=False, indent=2))
        return
    ...
    rows = []
    for source in sources:
        result = estimate_semantic_evidence_v0_2_1(source, as_of=args.as_of)
    ...
    artifact = {
        "name": "raos-semantic-evidence-development-run-v0.2.1",
        "status": "DEVELOPMENT_ONLY_NOT_FRESH_VALIDATION",
        "measurement_timestamp": timestamp,
        "measurement_git_head": git_head(),
        "as_of": args.as_of,
        "invocation": invocation_record(requested_model=settings.llm_model, provider_base_url=settings.llm_base_url),
        "n_sources": len(rows),
    ...
    out_path = args.out_dir / f"semantic_evidence_dev_v0_2_1_{timestamp}.json"
    if out_path.exists():
    ...
    print(json.dumps({
        ...
        "results": [{
            ...
            "source_published_at": row["source"]["published_at"],
