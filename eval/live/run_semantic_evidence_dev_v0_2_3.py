"""Run Semantic Evidence Extractor v0.2.3 minimal-sufficient candidate.
...
DEFAULT_OUT_DIR = ROOT / "eval" / "live" / "results" / "semantic_evidence_dev_v0_2_3"
...
def main() -> None:
    ...
    rows = []
    for source in sources:
        result = estimate_semantic_evidence_v0_2_3(source, as_of=args.as_of)
    ...
    artifact = {
        "name": "raos-semantic-evidence-development-run-v0.2.3-minimal-sufficient",
    ...
    out_path = args.out_dir / f"semantic_evidence_dev_v0_2_3_{timestamp}.json"
    if out_path.exists():
