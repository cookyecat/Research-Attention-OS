"""Run Standing Radar Fit (D) estimator v2 eval. Not part of ordinary CI."""
...
DEFAULT_MANIFEST = ROOT / "eval" / "live" / "manifest.standing_radar_fit_v2_fresh_human_gold.yaml"
DEFAULT_OUT = ROOT / "eval" / "live" / "results" / "standing_radar_fit_v2_first_fresh_run.json"
...
def run_manifest_v2(
    ...
    *,
    ...
    for case in manifest["cases"]:
        ...
        estimate = estimate_standing_radar_fit_v2(event, profile=profile, chat_fn=chat_fn)
        row["scorable"] = bool(estimate["scorable"])
        row["prediction"] = estimate["standing_radar_fit"]
        row["substantive_facets"] = estimate["substantive_facets"]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Standing Radar Fit (D) estimator v2 eval")
    ...
    payload = run_manifest_v2(args.manifest, score=not args.no_score, dry_run=args.dry_run)
    if args.dry_run:
