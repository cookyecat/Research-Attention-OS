...
RUN_VERSION = "phase17-jev-longitudinal-state-replay-v0.1"
...
OUT_DIR = ROOT / "eval/live/results/phase17_jev_longitudinal_state_replay_v0_1"
...
def _latest_full_hydration() -> Path:
    rows = sorted(HYDRATION_DIR.glob("*_full_*.json"))
    if not rows:
        raise RuntimeError(
            "No full Jev semantic hydration artifact found. "
            "Run run_phase17_jev_semantic_hydration_v0_1.py first."
        )
    return rows[-1]
...
def _load_source_graph(
    ...
    selected_rows = [
        row for row in hydration["items"]
...
def _observation(
    ...
    *,
    ...
    units = list(hydration_row.get("event_units") or [])
    unit_refs = tuple(
        sorted(
            {
...
def run(
    *,
    limit: int | None = None,
    hydration_path: Path | None = None,
) -> dict:
    hydration_path = (hydration_path or _latest_full_hydration()).resolve()
    hydration = json.loads(hydration_path.read_text())
    fixture = json.loads(FIXTURE.read_text())

    if hydration["summary"]["failed"] or hydration["summary"]["no_source"]:
        raise RuntimeError(
            "Full hydration contains failed/no-source rows; longitudinal replay "
            "requires an explicit missing-evidence policy first."
        )

    by_source_id, source_clones, edge_clones = _load_source_graph(hydration)
    ...
    try:
        ...
        for ordinal, row in enumerate(hydrated_rows, start=1):
            ...
            new_units = list(row.get("event_units") or [])
            if not new_units:
                raise RuntimeError(
                    f"Hydrated Source {source_id} has zero Event semantic units"
                )
        ...
        return {
            ...
            "hydration_run_version": hydration["run_version"],
            ...
            "guardrails": [
                "Full frozen hydration artifact required.",
                ...
                "Configured real LLM is used only for Phi; deterministic R retains state authority.",
...
def main():
    ...
    report = run(
        limit=args.limit,
        hydration_path=args.hydration_artifact,
    )
