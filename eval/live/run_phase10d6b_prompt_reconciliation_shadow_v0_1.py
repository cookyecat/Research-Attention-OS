...
RUN_VERSION = "phase10d6b-prompt-reconciliation-shadow-v0.1"
OUT_DIR = ROOT / "eval/live/results/phase10d6b_prompt_reconciliation_shadow_v0_1"
...
def run_one(label: str, arm: str, ordinal: int, case: dict, nodes, matches, strategy) -> dict:
    system = IMPACT_SYSTEM if arm == "P0" else IMPACT_SYSTEM_VNEXT
    version = "production-impact-v2.1-legacy" if arm == "P0" else "canonical-impact-vnext-v0.1"
...
def main() -> int:
    ...
    out = {
        ...
        "status": "PROMPT_ONLY_SHADOW_AB",
        "measurement_sha": git_head(),
        "repeats_per_arm": REPEATS,
        "cases": list(CASES),
        "prompt_sha256": {"P0": sha_text(IMPACT_SYSTEM), "P1": sha_text(IMPACT_SYSTEM_VNEXT)},
        "decision_strategy": strategy.execution_snapshot(),
        "controlled_variable": "impact_system_prompt_only",
        "results": results,
        "guardrails": [
            ...
            "Only the Impact system prompt differs between P0 and P1.",
