...
## Dogfood Attention UI — SOURCE-CENTRIC FEED / HYDRATION REPAIR CLOSED 2026-09-13

Real use exposed that the Attention page was rendering the raw `AttentionPlan` ledger rather than user-consumable information objects: repeated plans from the same Source appeared as duplicated one-line judgments, with no article title, provenance context, or direct navigation to the analyzed Source. Commit `b4175dddd97ac5ff9b6ffd3ff8166d3682746042` (`ui: make attention feed source-centric`) repairs the presentation layer without changing cognition semantics: plans are grouped by `candidate_type + candidate_id`, only the latest current plan per Source is shown, Source title/origin/time are displayed, cards navigate to `/attention?source=<source_id>`, URL-backed details expose the original article link, and DROP remains hidden by default behind an explicit toggle.
...
## Reader UX V3 dogfood correction — SPARSE EVIDENCE + MODAL ISOLATION CLOSED 2026-09-14
...
### Phase 11B — Query Expansion / Active Acquisition — CLOSED 2026-09-15
...
### Phase 10E — Core Decision-State Sufficiency Gate — ACTIVE 2026-09-15
...
Phase 10E is validation-only, not a new runtime module. Pareto/Magnitude-Free/Stability re-review is recorded in `213_PHASE10E_PARETO_ORDER_COHERENCE_REVIEW.md`. The first gate is now deterministic policy-order coherence: Pareto dominance must be compatible with per-effect Attention severity before any replay or Human-Gold interpretation. An exploratory exhaustive two-effect audit found 25/946 synthetic pairs where current Pareto pruning changes final disposition; the pattern concentrates on `REINFORCE(DECISION)` because `active_role_band` merges DECISION with QUESTION/BOTTLENECK while channel policy treats DECISION as immediate ENGAGE. No such target occurs in the three current-strategy non-empty dogfood runs, so production reachability remains unproven.

Only after this algebra/reachability gate passes does Phase 10E proceed to strategy-explicit replay parity and then the minimal state-sufficiency Human-Gold question. Historical Human Gold may suggest matched-pair probes, but it is not current calibration Gold. Any reproducible violation creates a candidate Core-insufficiency hypothesis; it does not automatically authorize a new variable or chip.
