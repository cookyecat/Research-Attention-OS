...
The historical `eval/live/oracle_policy.py` issue is now scoped precisely: Oracle-Delta was an evaluation-only policy-isolation harness introduced before the pluggable decision-strategy seam. It bare-calls `scheduler.route()`, which later preserved `one-delta-v1` as a compatibility default. This makes old Oracle-Delta / Oracle-Awareness policy-isolation results historical with respect to the current Phase-10 strategy; it does **not** imply that real acquisition/frontend/continuous-arrival/Phase-11 dogfood silently used the legacy strategy. The production pipeline resolves and snapshots an explicit decision strategy before routing.
